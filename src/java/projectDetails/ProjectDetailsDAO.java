...
public class ProjectDetailsDAO {
    public List<ProjectDetailsDTO> searchListProjectDetailsByName(String searchProjectName) throws SQLException{
        ...
        try{
            ...
            if(con != null){
                ...
                "WHERE Project.status = ? AND Project.projectName like ? ";
                stm = con.prepareStatement(sql);
                stm.setInt(1, 1);
                stm.setString(2, "%" + searchProjectName + "%");
                rs = stm.executeQuery();
                list = new ArrayList<>();
                while(rs.next()){
    ...
    public List<ProjectDetailsDTO> listProjectDetails(String searchProjectName) throws SQLException{
        List<ProjectDetailsDTO> list = null;
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        try{
            ...
            if(con != null){
                ...
                while(rs.next()){
                    ...
                    ProjectDetailsDTO dto = new ProjectDetailsDTO(projectID, projectName, scale, description, image, createAt, updateAt, price/100, userID, firstName, lastName, projectTypeID, projectTypeName, status);
                    list.add(dto);
                }
            }
        }catch(Exception e){
